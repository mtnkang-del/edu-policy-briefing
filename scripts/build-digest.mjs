...
function fallbackDigest(data){
  const items=[...(data.items||[])].sort((a,b)=>(b.score||0)-(a.score||0));
  const counts=new Map();
  for(const x of items) counts.set(x.category||'기타',(counts.get(x.category||'기타')||0)+1);
  ...
  const issues=chosen.map(cat=>{
    const rows=items.filter(x=>(x.category||'기타')===cat).slice(0,3);
    return {
      title:cat,
      summary:rows.map(x=>clean(x.title)).filter(Boolean).join(' · '),
      why:whyByCategory[cat]||whyByCategory['기타'],
      sources:rows.map(x=>x.source).filter(Boolean)
    };
  });
  const topTitles=items.slice(0,3).map(x=>clean(x.title)).filter(Boolean);
  const focus=chosen.slice(0,3).join('·')||'주요 교육정책';
  return {
    ai:false,
    headline:`${data.date} 교육뉴스는 ${focus} 이슈를 중심으로 형성됐습니다.`,
    summary:`오늘 수집된 교육 관련 자료는 총 ${data.stats?.total||items.length}건입니다. 교육부 공식 ${data.stats?.official||0}건, 국내 언론 ${data.stats?.domestic||0}건, 해외 언론 ${data.stats?.foreign||0}건을 종합하면 ${topTitles.join(' / ')} 등이 주요 흐름으로 확인됩니다.`,
    issues,
    watchpoints:issues.slice(0,3).map(x=>`${x.title}: ${x.why}`)
  };
}

async function aiDigest(data,fallback){
  if(!process.env.OPENAI_API_KEY) return fallback;
  try{
    const OpenAI=(await import('openai')).default;
    const client=new OpenAI({apiKey:process.env.OPENAI_API_KEY});
    const rows=(data.items||[]).slice(0,35).map((x,i)=>({i,title:x.title,titleKo:x.titleKo||'',source:x.source,kind:x.kind,category:x.category,description:x.description||'',summary:x.summary||x.summaryKo||''}));
    const prompt=`너는 대한민국 교육부 정책 담당자를 위한 데일리 브리핑 편집자다. 아래 당일 뉴스 목록만 근거로 같은 사건의 중복 보도를 하나의 이슈로 묶어 한국어 브리핑을 작성하라. 사실을 추가하거나 추측하지 마라. JSON만 출력한다. 형식은 {"headline":"오늘 전체를 한 문장으로 총평","summary":"전체 흐름을 3~4문장으로 요약","issues":[{"title":"이슈명","summary":"무슨 일이 있었는지 2문장 이내","why":"교육부가 왜 봐야 하는지 1문장","sources":["출처"]}],"watchpoints":["오늘 확인할 점"]}. issues는 최대 5개, watchpoints는 최대 4개. 기사 목록: ${JSON.stringify(rows)}`;
...
const fallback=fallbackDigest(data);
data.dailyDigest=await aiDigest(data,fallback);
...
console.log(`Daily digest built. AI=${data.dailyDigest.ai}`);
